...
namespace PainelSeguranca.Core
{
    ...
    public static class I18n
    {
        ...
        private static readonly Dictionary<string, string> Pt = new Dictionary<string, string>
        {
            ...
            { "Main.Tab.Blocked", "Bloqueados na internet" },
            { "Main.Tab.Exclude", "Nao verificar" },
            { "Main.Tab.Protect", "Protecao" },
            { "Main.Blocked.Hint", "Apps que o firewall esta IMPEDINDO de acessar a internet. Selecione um e clique Liberar para devolver o acesso." },
            { "Main.Blocked.Empty", "Nenhum app bloqueado no momento." },
            { "Main.Blocked.Add", "Bloquear um app..." },
            { "Main.Blocked.Allow", "Liberar selecionado" },
            { "Main.Exclude.Hint", "Arquivos e pastas que o Defender NAO verifica. Adicione o que e seguro (ex.: a pasta do seu projeto)." },
            { "Main.Exclude.Empty", "Nenhuma exclusao. O Defender verifica tudo." },
            { "Main.Exclude.AddFile", "Adicionar arquivo..." },
            { "Main.Exclude.AddFolder", "Adicionar pasta..." },
            { "Main.Exclude.Remove", "Remover selecionado" },
            { "Main.Exclude.Col", "Arquivo ou pasta" },
            { "Main.NeedSelectExcl", "Selecione um arquivo ou pasta da lista primeiro." },
            ...
            { "Adv.Tab.Blocked", "Bloqueados" },
            { "Adv.Tab.Allowed", "Liberados" },

            { "Conn.Blocked.Hint", "Programas que o firewall esta IMPEDINDO de acessar a rede (regras ativas de bloqueio)." },
            { "Conn.Allowed.Hint", "Programas que o firewall esta PERMITINDO acessar a rede (regras ativas de liberacao)." },
            { "Conn.Col.Program", "Programa" },
            { "Conn.Col.Rule", "Regra (nome)" },
            { "Conn.Col.Profile", "Perfil" },
            { "Conn.Count", "{0} item(ns)" },
            { "Conn.Loading", "Carregando..." },
            { "Conn.Btn.Block", "Bloquear conexao do app selecionado" },
            { "Conn.Btn.Allow", "Liberar conexao do app selecionado" },
            { "Conn.ConfirmBlock", "Bloquear o acesso a internet de \"{0}\"?" },
            { "Conn.ConfirmAllow", "Liberar a conexao de \"{0}\"?\n\nIsso remove a regra de bloqueio selecionada, e o programa volta a poder se conectar." },
            { "Conn.NeedSelect", "Selecione um item da lista primeiro." },
        ...
        private static readonly Dictionary<string, string> En = new Dictionary<string, string>
        {
            ...
            { "Main.Tab.Blocked", "Blocked on the internet" },
            { "Main.Tab.Exclude", "Not scanned" },
            { "Main.Tab.Protect", "Protection" },
            { "Main.Blocked.Hint", "Apps the firewall is BLOCKING from the internet. Select one and click Allow to give access back." },
            { "Main.Blocked.Empty", "No apps blocked right now." },
            { "Main.Blocked.Add", "Block an app..." },
            { "Main.Blocked.Allow", "Allow selected" },
            { "Main.Exclude.Hint", "Files and folders the Defender does NOT scan. Add what is safe (e.g. your project folder)." },
            { "Main.Exclude.Empty", "No exclusions. Defender scans everything." },
            { "Main.Exclude.AddFile", "Add file..." },
            { "Main.Exclude.AddFolder", "Add folder..." },
            { "Main.Exclude.Remove", "Remove selected" },
            { "Main.Exclude.Col", "File or folder" },
            { "Main.NeedSelectExcl", "Select a file or folder from the list first." },
            ...
            { "Adv.Tab.Blocked", "Blocked" },
            { "Adv.Tab.Allowed", "Allowed" },

            { "Conn.Blocked.Hint", "Programs the firewall is BLOCKING from accessing the network (active block rules)." },
            { "Conn.Allowed.Hint", "Programs the firewall is ALLOWING to access the network (active allow rules)." },
            { "Conn.Col.Program", "Program" },
            { "Conn.Col.Rule", "Rule (name)" },
            { "Conn.Col.Profile", "Profile" },
            { "Conn.Count", "{0} item(s)" },
            { "Conn.Loading", "Loading..." },
            { "Conn.Btn.Block", "Block selected app's connection" },
            { "Conn.Btn.Allow", "Allow selected app's connection" },
            { "Conn.ConfirmBlock", "Block internet access for \"{0}\"?" },
            { "Conn.ConfirmAllow", "Allow the connection for \"{0}\"?\n\nThis removes the selected block rule, so the program can connect again." },
            { "Conn.NeedSelect", "Select an item from the list first." },
